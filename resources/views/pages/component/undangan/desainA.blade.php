<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">

    @vite('resources/css/app.css')
</head>

<body>
    
    <header class="h-screen bg-fixed bg-center bg-cover custom-img mb-10">
        <div class="grid place-items-center">
        </div>
        <div class="flex h-screen">
            <div class="m-auto text-center">
                <p
                    class="text-xl text-gray-800 font-rale-way lg:text-3xl sm:text-xl font-bold sm:px-16 dark:text-gray-400">
                    We Are Getting Married</p>
                <p
                    class="leading-7 my-28 font-great-vibes lg:text-9xl text-7xl tracking-tight text-gray-900 dark:text-white ">
                    {{ $data['groom']['name']}} & {{ $data['bride']['name'] }}</p>
                <p class="font-normal text-gray-500 text-xl sm:px-16 dark:text-gray-400 font-alegreya">Kepada Yth
                </p>
                <p class="font-normal text-gray-500 text-xl sm:px-16 dark:text-gray-400 font-alegreya">
                    Bapak/Ibu/Saudara/i</p>
                {{-- <p class="font-bold text-slate-800 text-2xl sm:px-16 dark:text-gray-400 font-alegreya mt-10">Wisnu
                    Yuwono</p> --}}
            </div>
        </div>
    </header>

    <section id="bottom-navigation" class="block fixed inset-x-0 bottom-0 z-10 bg-white shadow">
        <div id="tabs" class="flex justify-between">
            <a href=""
                class="w-full focus:text-teal-500 hover:text-teal-500 justify-center inline-block text-center pt-2 pb-1">
                <svg width="25" height="25" viewBox="0 0 42 42" class="inline-block mb-1">
                    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <path
                            d="M21.0847458,3.38674884 C17.8305085,7.08474576 17.8305085,10.7827427 21.0847458,14.4807396 C24.3389831,18.1787365 24.3389831,22.5701079 21.0847458,27.6548536 L21.0847458,42 L8.06779661,41.3066256 L6,38.5331279 L6,26.2681048 L6,17.2542373 L8.88135593,12.4006163 L21.0847458,2 L21.0847458,3.38674884 Z"
                            fill="currentColor" fillOpacity="0.1"></path>
                        <path
                            d="M11,8 L33,8 L11,8 Z M39,17 L39,36 C39,39.3137085 36.3137085,42 33,42 L11,42 C7.6862915,42 5,39.3137085 5,36 L5,17 L7,17 L7,36 C7,38.209139 8.790861,40 11,40 L33,40 C35.209139,40 37,38.209139 37,36 L37,17 L39,17 Z"
                            fill="currentColor"></path>
                        <path
                            d="M22,27 C25.3137085,27 28,29.6862915 28,33 L28,41 L16,41 L16,33 C16,29.6862915 18.6862915,27 22,27 Z"
                            stroke="currentColor" strokeWidth="2" fill="currentColor" fillOpacity="0.1"></path>
                        <rect fill="currentColor"
                            transform="translate(32.000000, 11.313708) scale(-1, 1) rotate(-45.000000) translate(-32.000000, -11.313708) "
                            x="17" y="10.3137085" width="30" height="2" rx="1"></rect>
                        <rect fill="currentColor"
                            transform="translate(12.000000, 11.313708) rotate(-45.000000) translate(-12.000000, -11.313708) "
                            x="-3" y="10.3137085" width="30" height="2" rx="1"></rect>
                    </g>
                </svg>
                <span class="tab tab-home block text-xs">Home</span>
            </a>
            <a href="#couple"
                class="w-full focus:text-teal-500 hover:text-teal-500 justify-center inline-block text-center pt-2 pb-1">
                <svg width="25" height="25" viewBox="0 0 42 42" class="inline-block mb-1">
                    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <path
                            d="M14.7118754,20.0876892 L8.03575361,20.0876892 C5.82661462,20.0876892 4.03575361,18.2968282 4.03575361,16.0876892 L4.03575361,12.031922 C4.03575361,8.1480343 6.79157254,4.90780265 10.4544842,4.15995321 C8.87553278,8.5612583 8.1226025,14.3600511 10.9452499,15.5413938 C13.710306,16.6986332 14.5947501,18.3118357 14.7118754,20.0876892 Z M14.2420017,23.8186831 C13.515543,27.1052019 12.7414284,30.2811559 18.0438552,31.7330419 L18.0438552,33.4450645 C18.0438552,35.6542035 16.2529942,37.4450645 14.0438552,37.4450645 L9.90612103,37.4450645 C6.14196811,37.4450645 3.09051926,34.3936157 3.09051926,30.6294627 L3.09051926,27.813861 C3.09051926,25.604722 4.88138026,23.813861 7.09051926,23.813861 L14.0438552,23.813861 C14.1102948,23.813861 14.1763561,23.8154808 14.2420017,23.8186831 Z M20.7553776,32.160536 C23.9336213,32.1190063 23.9061943,29.4103976 33.8698747,31.1666916 C34.7935223,31.3295026 35.9925894,31.0627305 37.3154077,30.4407183 C37.09778,34.8980343 33.4149547,38.4450645 28.9036761,38.4450645 C24.9909035,38.4450645 21.701346,35.7767637 20.7553776,32.160536 Z"
                            fill="currentColor" opacity="0.1"></path>
                        <g transform="translate(2.000000, 3.000000)">
                            <path
                                d="M8.5,1 C4.35786438,1 1,4.35786438 1,8.5 L1,13 C1,14.6568542 2.34314575,16 4,16 L13,16 C14.6568542,16 16,14.6568542 16,13 L16,4 C16,2.34314575 14.6568542,1 13,1 L8.5,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M4,20 C2.34314575,20 1,21.3431458 1,23 L1,27.5 C1,31.6421356 4.35786438,35 8.5,35 L13,35 C14.6568542,35 16,33.6568542 16,32 L16,23 C16,21.3431458 14.6568542,20 13,20 L4,20 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M23,1 C21.3431458,1 20,2.34314575 20,4 L20,13 C20,14.6568542 21.3431458,16 23,16 L32,16 C33.6568542,16 35,14.6568542 35,13 L35,8.5 C35,4.35786438 31.6421356,1 27.5,1 L23,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M34.5825451,33.4769886 L38.3146092,33.4322291 C38.8602707,33.4256848 39.3079219,33.8627257 39.3144662,34.4083873 C39.3145136,34.4123369 39.3145372,34.4162868 39.3145372,34.4202367 L39.3145372,34.432158 C39.3145372,34.9797651 38.8740974,35.425519 38.3265296,35.4320861 L34.5944655,35.4768456 C34.048804,35.4833899 33.6011528,35.046349 33.5946085,34.5006874 C33.5945611,34.4967378 33.5945375,34.4927879 33.5945375,34.488838 L33.5945375,34.4769167 C33.5945375,33.9293096 34.0349773,33.4835557 34.5825451,33.4769886 Z"
                                fill="currentColor"
                                transform="translate(36.454537, 34.454537) rotate(-315.000000) translate(-36.454537, -34.454537) ">
                            </path>
                            <circle stroke="currentColor" strokeWidth="2" cx="27.5" cy="27.5" r="7.5"></circle>
                        </g>
                    </g>
                </svg>
                <span class="tab tab-kategori block text-xs">Couple</span>
            </a>
            <a href="#events"
                class="w-full focus:text-teal-500 hover:text-teal-500 justify-center inline-block text-center pt-2 pb-1">
                <svg width="25" height="25" viewBox="0 0 42 42" class="inline-block mb-1">
                    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <path
                            d="M20.5890101,0.254646884 C12.8696785,5.50211755 8.0025785,14.258415 14.1941217,18.8708225 C23.16683,25.5550669 13.3362326,40.2698884 33.1021758,38.4149164 C29.6814884,40.8311956 25.5065164,42.2507054 21,42.2507054 C9.40202025,42.2507054 0,32.8486852 0,21.2507054 C0,9.79003409 9.18071714,0.473634138 20.5890101,0.254646884 Z"
                            fill="currentColor" opacity="0.1"></path>
                        <path
                            d="M25.9500282,20.3643496 L22.4308312,38.2677802 C22.3775703,38.5387376 22.1147395,38.7152155 21.8437821,38.6619546 C21.6570955,38.6252584 21.507413,38.4857901 21.4576354,38.3021581 L16.5951895,20.3643496 L20.099732,4.44663907 C20.1385204,4.27046145 20.2692032,4.12883813 20.4417012,4.07604096 C20.7057521,3.99522179 20.9853245,4.14376046 21.0661436,4.40781135 L25.9500282,20.3643496 Z M21.3022963,22.2852638 C22.4068658,22.2852638 23.3022963,21.3898333 23.3022963,20.2852638 C23.3022963,19.1806943 22.4068658,18.2852638 21.3022963,18.2852638 C20.1977268,18.2852638 19.3022963,19.1806943 19.3022963,20.2852638 C19.3022963,21.3898333 20.1977268,22.2852638 21.3022963,22.2852638 Z"
                            fill="currentColor"
                            transform="translate(21.272609, 20.629524) rotate(-315.000000) translate(-21.272609, -20.629524) ">
                        </path>
                        <circle stroke="currentColor" strokeWidth="2" cx="21" cy="21" r="20"></circle>
                    </g>
                </svg>
                <span class="tab tab-explore block text-xs">Events</span>
            </a>
            <a href="#gallery"
                class="w-full focus:text-teal-500 hover:text-teal-500 justify-center inline-block text-center pt-2 pb-1">
                <svg width="25" height="25" viewBox="0 0 42 42" class="inline-block mb-1">
                    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <path
                            d="M14.7118754,20.0876892 L8.03575361,20.0876892 C5.82661462,20.0876892 4.03575361,18.2968282 4.03575361,16.0876892 L4.03575361,12.031922 C4.03575361,8.1480343 6.79157254,4.90780265 10.4544842,4.15995321 C8.87553278,8.5612583 8.1226025,14.3600511 10.9452499,15.5413938 C13.710306,16.6986332 14.5947501,18.3118357 14.7118754,20.0876892 Z M14.2420017,23.8186831 C13.515543,27.1052019 12.7414284,30.2811559 18.0438552,31.7330419 L18.0438552,33.4450645 C18.0438552,35.6542035 16.2529942,37.4450645 14.0438552,37.4450645 L9.90612103,37.4450645 C6.14196811,37.4450645 3.09051926,34.3936157 3.09051926,30.6294627 L3.09051926,27.813861 C3.09051926,25.604722 4.88138026,23.813861 7.09051926,23.813861 L14.0438552,23.813861 C14.1102948,23.813861 14.1763561,23.8154808 14.2420017,23.8186831 Z M20.7553776,32.160536 C23.9336213,32.1190063 23.9061943,29.4103976 33.8698747,31.1666916 C34.7935223,31.3295026 35.9925894,31.0627305 37.3154077,30.4407183 C37.09778,34.8980343 33.4149547,38.4450645 28.9036761,38.4450645 C24.9909035,38.4450645 21.701346,35.7767637 20.7553776,32.160536 Z"
                            fill="currentColor" opacity="0.1"></path>
                        <g transform="translate(2.000000, 3.000000)">
                            <path
                                d="M8.5,1 C4.35786438,1 1,4.35786438 1,8.5 L1,13 C1,14.6568542 2.34314575,16 4,16 L13,16 C14.6568542,16 16,14.6568542 16,13 L16,4 C16,2.34314575 14.6568542,1 13,1 L8.5,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M4,20 C2.34314575,20 1,21.3431458 1,23 L1,27.5 C1,31.6421356 4.35786438,35 8.5,35 L13,35 C14.6568542,35 16,33.6568542 16,32 L16,23 C16,21.3431458 14.6568542,20 13,20 L4,20 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M23,1 C21.3431458,1 20,2.34314575 20,4 L20,13 C20,14.6568542 21.3431458,16 23,16 L32,16 C33.6568542,16 35,14.6568542 35,13 L35,8.5 C35,4.35786438 31.6421356,1 27.5,1 L23,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M34.5825451,33.4769886 L38.3146092,33.4322291 C38.8602707,33.4256848 39.3079219,33.8627257 39.3144662,34.4083873 C39.3145136,34.4123369 39.3145372,34.4162868 39.3145372,34.4202367 L39.3145372,34.432158 C39.3145372,34.9797651 38.8740974,35.425519 38.3265296,35.4320861 L34.5944655,35.4768456 C34.048804,35.4833899 33.6011528,35.046349 33.5946085,34.5006874 C33.5945611,34.4967378 33.5945375,34.4927879 33.5945375,34.488838 L33.5945375,34.4769167 C33.5945375,33.9293096 34.0349773,33.4835557 34.5825451,33.4769886 Z"
                                fill="currentColor"
                                transform="translate(36.454537, 34.454537) rotate(-315.000000) translate(-36.454537, -34.454537) ">
                            </path>
                            <circle stroke="currentColor" strokeWidth="2" cx="27.5" cy="27.5" r="7.5"></circle>
                        </g>
                    </g>
                </svg>
                <span class="tab tab-whishlist block text-xs">Gallery</span>
            </a>
            <a href="#guest-book"
                class="w-full focus:text-teal-500 hover:text-teal-500 justify-center inline-block text-center pt-2 pb-1">
                <svg width="25" height="25" viewBox="0 0 42 42" class="inline-block mb-1">
                    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <path
                            d="M14.7118754,20.0876892 L8.03575361,20.0876892 C5.82661462,20.0876892 4.03575361,18.2968282 4.03575361,16.0876892 L4.03575361,12.031922 C4.03575361,8.1480343 6.79157254,4.90780265 10.4544842,4.15995321 C8.87553278,8.5612583 8.1226025,14.3600511 10.9452499,15.5413938 C13.710306,16.6986332 14.5947501,18.3118357 14.7118754,20.0876892 Z M14.2420017,23.8186831 C13.515543,27.1052019 12.7414284,30.2811559 18.0438552,31.7330419 L18.0438552,33.4450645 C18.0438552,35.6542035 16.2529942,37.4450645 14.0438552,37.4450645 L9.90612103,37.4450645 C6.14196811,37.4450645 3.09051926,34.3936157 3.09051926,30.6294627 L3.09051926,27.813861 C3.09051926,25.604722 4.88138026,23.813861 7.09051926,23.813861 L14.0438552,23.813861 C14.1102948,23.813861 14.1763561,23.8154808 14.2420017,23.8186831 Z M20.7553776,32.160536 C23.9336213,32.1190063 23.9061943,29.4103976 33.8698747,31.1666916 C34.7935223,31.3295026 35.9925894,31.0627305 37.3154077,30.4407183 C37.09778,34.8980343 33.4149547,38.4450645 28.9036761,38.4450645 C24.9909035,38.4450645 21.701346,35.7767637 20.7553776,32.160536 Z"
                            fill="currentColor" opacity="0.1"></path>
                        <g transform="translate(2.000000, 3.000000)">
                            <path
                                d="M8.5,1 C4.35786438,1 1,4.35786438 1,8.5 L1,13 C1,14.6568542 2.34314575,16 4,16 L13,16 C14.6568542,16 16,14.6568542 16,13 L16,4 C16,2.34314575 14.6568542,1 13,1 L8.5,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M4,20 C2.34314575,20 1,21.3431458 1,23 L1,27.5 C1,31.6421356 4.35786438,35 8.5,35 L13,35 C14.6568542,35 16,33.6568542 16,32 L16,23 C16,21.3431458 14.6568542,20 13,20 L4,20 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M23,1 C21.3431458,1 20,2.34314575 20,4 L20,13 C20,14.6568542 21.3431458,16 23,16 L32,16 C33.6568542,16 35,14.6568542 35,13 L35,8.5 C35,4.35786438 31.6421356,1 27.5,1 L23,1 Z"
                                stroke="currentColor" strokeWidth="2"></path>
                            <path
                                d="M34.5825451,33.4769886 L38.3146092,33.4322291 C38.8602707,33.4256848 39.3079219,33.8627257 39.3144662,34.4083873 C39.3145136,34.4123369 39.3145372,34.4162868 39.3145372,34.4202367 L39.3145372,34.432158 C39.3145372,34.9797651 38.8740974,35.425519 38.3265296,35.4320861 L34.5944655,35.4768456 C34.048804,35.4833899 33.6011528,35.046349 33.5946085,34.5006874 C33.5945611,34.4967378 33.5945375,34.4927879 33.5945375,34.488838 L33.5945375,34.4769167 C33.5945375,33.9293096 34.0349773,33.4835557 34.5825451,33.4769886 Z"
                                fill="currentColor"
                                transform="translate(36.454537, 34.454537) rotate(-315.000000) translate(-36.454537, -34.454537) ">
                            </path>
                            <circle stroke="currentColor" strokeWidth="2" cx="27.5" cy="27.5" r="7.5"></circle>
                        </g>
                    </g>
                </svg>
                <span class="tab tab-account block text-xs">Guest Book</span>
            </a>
        </div>
    </section>

    <div id='couple' class="my-10">
        <p class='text-black text-center font-great-vibes text-5xl pb-14'>Pasangan Pengantin</p>
        <div class="max-w-screen-lg mx-auto grid grid-cols-1 lg:grid-cols-2 sm:grid-cols-2 gap-4">
            <div class="flex justify-center items-center">
                <div class="container flex justify-center">
                    <div class="max-w-sm">
                        <div class="bg-white relative shadow-lg hover:shadow-xl transition duration-500 rounded-lg">
                            <img class="rounded-t-lg"
                                src="{{asset('images/groom/'.$data['groom']->image)}}" alt="" />
                            <div class="py-6 px-8 rounded-lg bg-white">
                                <p class="text-gray-700 tracking-wide text-justify">Anak dari {{ $data['groom']['father'] }} dan {{ $data['groom']['mother'] }}</p>
                                <p
                                    class="text-gray-700 font-bold font-tangerine text-5xl mb-3 hover:text-gray-900 hover:cursor-pointer mt-5">
                                    {{ $data['groom']['name'] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex justify-center items-center">
                <div class="container flex justify-center">
                    <div class="max-w-sm">
                        <div class="bg-white relative shadow-lg hover:shadow-xl transition duration-500 rounded-lg">
                            <img class="rounded-t-lg"
                                src="{{asset('images/bride/'.$data['bride']->image)}}" alt="" />
                            <div class="py-6 px-8 rounded-lg bg-white">
                                <p class="text-gray-700 tracking-wide text-justify">Anak dari {{ $data['bride']['father'] }} dan {{ $data['bride']['mother'] }}</p>
                                <p
                                    class="text-gray-700 font-bold font-tangerine text-5xl mb-3 hover:text-gray-900 hover:cursor-pointer mt-5">
                                    {{ $data['bride']['name'] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div id='events' class="my-10">
        <p class='text-black text-center font-great-vibes text-5xl lg:pb-14'>Wedding Events</p>
        <div class="max-w-screen-2xl mx-auto sm:mb-10 lg:grid lg:grid-cols-5 md:grid-cols-none bg-white lg:h-full">
            <div
                class=" px-10 max-w-md m-auto lg:col-span-2 mt-10 mb-20 shadow-xl rounded-xl lg:mt-10 md:shadow-xl md:rounded-xl lg:shadow-none lg:rounded-none lg:w-full lg:mb-10 lg:px-5 lg:pt-5 lg:pb-5 lg:max-w-lg bg-white">
                <div class="py-10">
                    <p class="my-7 font-bold lg:mt-7 text-3xl text-left">Wedding Party</p>
                    <div class="flex mb-4">
                        <img class='w-7 mr-2' src="https://cdn-icons-png.flaticon.com/512/684/684908.png" alt="" />
                        <p class="font-bold text-lg text-gray-800 text-left">{{ $data['place']['address'] }}</p>
                    </div>
                    <div class="flex">
                        <img class='w-7 mr-2' src="https://cdn-icons-png.flaticon.com/512/2886/2886665.png" alt="" />
                        <p class="font-bold text-lg text-gray-800 text-left">{{ $data['invitation']['date'] }}, {{ $data['invitation']['time_start'] }} - {{ $data['invitation']['time_end'] }}</p>
                    </div>
                    <p class="text-lg text-black text-justify pt-2">{{$data['place']['place_desc']}}</p>
                </div>
            </div>

            <div class="hidden relative lg:block  lg:col-span-3">
                <img class="absolute inset-0 w-full h-full object-cover object-center"
                    src="{{asset('images/place/'.$data['place']->image)}}"
                    alt="Ad- woman on a beach" />
            </div>
        </div>
    </div>

    <section id='gallery' class="overflow-hidden text-gray-700">
        <div class="container px-5 mx-auto lg:px-32 my-10">
            <p class='text-black text-center font-great-vibes text-5xl pb-14'>Gallery</p>
            <div class="flex flex-wrap -m-1 md:-m-2">
                <div class="flex flex-wrap lg:w-1/2">
                    <div class="w-1/2 p-1 md:p-2">
                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                    </div>
                    <div class="w-1/2 p-1 md:p-2">
                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                    </div>
                    <div class="w-full p-1 md:p-2">
                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                    </div>
                </div>
                <div class="flex flex-wrap lg:w-1/2">
                    <div class="w-full p-1 md:p-2">
                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                    </div>
                    <div class="w-1/2 p-1 md:p-2">
                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                    </div>
                    <div class="w-1/2 p-1 md:p-2">

                        <img alt="gallery" class="block object-cover object-center w-full h-full rounded-lg"
                            src="{{asset('images/gallery/'.$data['gallery']->name)}}" />
                            {{-- public_path('images/'.$data['gallery']['image']) --}}
                            {{-- url('images/'.$data['gallery']['image']) --}}
                            {{-- asset('images/'.$data['gallery']['image']) --}}
                            {{-- public/images/gallery/{{ $data['gallery']['image'] }} --}}
                    </div>
                </div>
            </div>
        </div>
    </section>
    
    <div class="flex items-center justify-center mb-10">
        <div class="rounded-lg bg-gray-200 p-5 w-1/3">
            <div class="flex">
                <span id="link"
                    class="w-full bg-white py-auto py-2.5 text-base font-semibold outline-0">http://127.0.0.1:8000/guest/{{ $data['invitation']->id }}</span>
                <span id="copy-btn"
                    class="bg-blue-500 p-2 rounded-tr-lg rounded-br-lg text-white font-semibold hover:bg-blue-800 transition-colors">Copy</span>
            </div>
        </div>
    </div>

    <div id="guestbook" class="container">
        <p class='text-black text-center font-great-vibes text-5xl my-8'>Buku Tamu</p>
        <div class="max-w-2xl mx-auto">
            <div class="bg-white text-gray-800 p-4 pr-0 rounded-md shadow mb-36">
                @if($data['total_guest'] != null)
                <div class="flex flex-col space-y-3 h-80 overflow-y-scroll">
                @foreach($data['guest'] as $g)
                {{-- Looping Data --}}
                    <div class="flex space-x-2 mr-4">
                        <div class="flex items-center justify-center space-x-2 text-left">
                            <div class="block">
                                <div class="bg-gray-100 w-auto rounded-xl px-4 pt-2 pb-2">
                                    <div>
                                        <span class="text-sm font-semibold">{{ $g->name }}</span>
                                        <span
                                            class="bg-green-100 text-green-800 mx-1 inline-block px-2 py-0.5 text-xs font-medium bg-green-100 rounded-md">
                                            {{ $g->presence }}
                                        </span>
                                    </div>
                                    <div class="text-sm">{{ $g->congratulation }}</div>
                                </div>
                                <div class="flex justify-start items-center text-xs w-full">
                                    <div class="font-semibold text-gray-700 px-2 flex items-center justify-center">
                                        <small>{{ $g->created_at }}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    @endforeach
                </div>
                {{-- @elseif(in_array($data['total_guest'], 1))
                    <div class="flex flex-col space-y-3 h-80 overflow-y-scroll">
                    <div class="flex space-x-2 mr-4">
                        <div class="flex items-center justify-center space-x-2 text-left">
                            <div class="block">
                                <div class="bg-gray-100 w-auto rounded-xl px-4 pt-2 pb-2">
                                    <div>
                                        <span class="text-sm font-semibold">{{ $data['guest']->name }}</span>
                                        <span
                                            class="bg-green-100 text-green-800 mx-1 inline-block px-2 py-0.5 text-xs font-medium bg-green-100 rounded-md">
                                            {{ $data['guest']->presence }}
                                        </span>
                                    </div>
                                    <div class="text-sm">{{ $data['guest']->congratulation }}</div>
                                </div>
                                <div class="flex justify-start items-center text-xs w-full">
                                    <div class="font-semibold text-gray-700 px-2 flex items-center justify-center">
                                        <small>{{ $data['guest']->created_at }}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div> --}}
                @else
                    <div class="flex flex-col space-y-3 h-80 overflow-y-scroll">
                        <div class="flex space-x-2 mr-4">
                            <div class="flex items-center justify-center space-x-2 text-left">
                                <div class="block">
                                    <div class="bg-gray-100 w-auto rounded-xl px-4 pt-2 pb-2">
                                        <div>
                                            <span class="text-sm font-semibold">Belum ada ucapan</span>
                                            <span
                                            class="bg-green-100 text-green-800 mx-1 inline-block px-2 py-0.5 text-xs font-medium bg-green-100 rounded-md">
                                            none
                                        </span>
                                    </div>
                                    {{-- <div class="text-sm">Belum ada Ucapan</div> --}}
                                </div>
                                <div class="flex justify-start items-center text-xs w-full">
                                    <div class="font-semibold text-gray-700 px-2 flex items-center justify-center">
                                        {{-- <small>--------------</small> --}}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                @endif
            </div>
        </div>
    </div>
</body>

</html>